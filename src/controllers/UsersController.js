...
class UsersController {
	...
	async index(req, res){
		...
		var SELECT = 'SELECT users.id, email, user_profile.name, user_profile.description, last_name, password,';
		SELECT += ' phone, picture_url, role_id, role.name as role_name, role.description';
		SELECT += ' as role_description, role.status as role_status, users.status as user_status, user_profile.status as user_profile_status,';
		SELECT += ' facebook, instagram, twitter, sex FROM users, user_profile, role WHERE users.role_id = role.id';
  ...
  async getProviders(req, res, next){
    var SELECT = 'SELECT users.id, email, user_profile.name, user_profile.description, last_name, password,';
		...
		SELECT += ' as role_description, role.status as role_status, users.status as user_status, user_profile.status as user_profile_status,';
		SELECT += ' facebook, instagram, twitter, sex FROM users, user_profile, role WHERE users.role_id = role.id';
		SELECT += ' AND users.id = user_profile.user_id and role.name = "Profesor" '
  ...
  async getStundents(req, res, next){
    var SELECT = 'SELECT users.id, email, user_profile.name, last_name, password,';
    SELECT += ' phone, picture_url, role_id, role.name as role_name, role.description';
    SELECT += ' as role_description, role.status as role_status, users.status as user_status, user_profile.status as user_profile_status,';
    SELECT += ' facebook, instagram, twitter, sex FROM users, user_profile, role WHERE users.role_id = role.id';
    SELECT += ' AND users.id = user_profile.user_id and role.name = "Estudiante" '
    ...
    if(data.role_id){
      INSERT += `, role_id) VALUES('${ data.email }', '${ bcrypt.hashSync(data.password, 10) }', '${ bcrypt.hashSync(data.confirm_password, 10) }', '${ data.role_id }' )`
    }else{
      INSERT += `) VALUES('${ data.email }', '${ bcrypt.hashSync(data.password, 10) }', '${ bcrypt.hashSync(data.confirm_password, 10) }')`
    }
    ...
    this.connection.query('SELECT picture_url FROM user_profile WHERE user_id = ?', id, (err, resultImage) => {
    ...
    image.mv('public/uploads/users/'+new_image_name, (err) => {
      ...
      if (err)
        ...
        this.connection.query(`UPDATE user_profile SET picture_url = '${ new_image_name }' where user_id = ${id} `, (errsql, result) => {
    ...
    this.connection.query(`UPDATE users as usr, user_profile as up SET ? WHERE up.user_id = usr.id AND usr.id = ${ id } `, body, (err, resultUpdate) => {
      ...
      this.connection.query(`SELECT users.id, email, user_profile.name, user_profile.adress, user_profile.description, user_profile.last_name, user_profile.linkedin, password, user_profile.phone, user_profile.picture_url, role_id, role.name as role_name, role.description as role_description, role.status as role_status, users.status as user_status, user_profile.status as user_profile_status, user_profile.facebook, user_profile.instagram, user_profile.twitter, user_profile.sex FROM users, user_profile, role WHERE users.role_id = role.id AND users.id = user_profile.user_id and users.id = ${ id }`, (err, resultGet) => {
  ...
  async desactivate(req, res, next) {
    ...
    this.connection.query(`UPDATE users, user_profile SET users.status=\'2\', user_profile.status=\'2\' WHERE users.id = user_profile.user_id AND users.id = ${ id } `, (err, result) => {
  ...
  async reactivate(req, res, next) {
    ...
    this.connection.query(`UPDATE users, user_profile SET users.status=\'1\', user_profile.status=\'1\' WHERE users.id = user_profile.user_id AND users.id = ${ id } `, (err, result) => {
